...
def create_folders(args):
    ...
    # 两阶段 run 需要固定 checkpoint 目录，便于脚本阶段 2 查找
    if getattr(args.general, 'name', None) in ('morph_tiny50_two_stage', 'morph_full_two_stage'):
        try:
            os.makedirs('checkpoints', exist_ok=True)
            os.makedirs('checkpoints/' + args.general.name, exist_ok=True)
        except OSError:
            pass
