...
from graphgps.loader.dataset.morphology_node import MorphologyNodeDataset
...
def load_dataset_master(format, name, dataset_dir):
    ...
    """
    if format == 'PyG-MorphologyNode':
        dataset = preformat_MorphologyNode(dataset_dir)
...
def preformat_MorphologyNode(dataset_dir):
    """Load morphology .pt graphs from dataset_dir, preprocess, random train/val/test split by graph."""
    dataset = MorphologyNodeDataset(root=dataset_dir)
    n = len(dataset)
    split_ratios = getattr(cfg.dataset, 'split', [0.8, 0.1, 0.1])
    rng = np.random.default_rng(seed=cfg.seed)
    idx = rng.permutation(n)
    train_end = int(split_ratios[0] * n)
    val_end = train_end + int(split_ratios[1] * n)
    dataset.split_idxs = [
        idx[:train_end].tolist(),
        idx[train_end:val_end].tolist(),
        idx[val_end:].tolist(),
    ]
    return dataset
