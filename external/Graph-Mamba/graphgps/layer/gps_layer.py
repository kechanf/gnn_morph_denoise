...
try:
    ...
    Mamba = None  # 未安装 mamba-ssm 时仅可使用 GatedGCN-only 等非 Mamba 配置
...
def _sinusoidal_dist_encoding(dist, dim_pe, device):
    """Radial PE: dist -> [sin(d*w_i), cos(d*w_i), ...]. 多尺度编码，梯度更顺。"""
    div_term = torch.exp(
        torch.arange(0, dim_pe, 2, device=device, dtype=dist.dtype)
        * (-math.log(10000.0) / dim_pe)
    )
    pe = torch.zeros(dist.shape[0], dim_pe, device=device, dtype=dist.dtype)
    pe[:, 0::2] = torch.sin(dist.unsqueeze(-1) * div_term)
    pe[:, 1::2] = torch.cos(dist.unsqueeze(-1) * div_term)
    return pe


def gnn_priority_lexsort(batch_vec, scores, dist_from_root):
    """Vectorized GPU sort: Key=(batch, dist, -score). 解决 BFS 短视 + Python 瓶颈。

    Key 含义：batch 分组，dist 主导（层序），-score 微调（同层内高分优先）。
    全 GPU 操作，无 Python 循环，无 .item() 同步。
    """
    # lexsort: 最后一维为主键。keys=[-scores, dist, batch] -> 主键 batch，次键 dist，再次 -score
    return lexsort([-scores, dist_from_root, batch_vec])
...
def gnn_priority_bfs_within_batch(edge_index, batch, scores):
    ...
    for b in unique_batches:
        ...
        # 未访问节点（不连通分量）按 score 降序排在末尾，保证 perm 覆盖全部 n_b 个节点
        unvisited = [i for i in range(n_b) if not visited[i]]
        if unvisited:
            unvisited_scores = torch.tensor([float(scores_b[i].item()) for i in unvisited], device=device)
            order_unvisited = [unvisited[i] for i in unvisited_scores.argsort(descending=True).tolist()]
            order_local = order_local + order_unvisited
...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        # Optional conflict-aware fusion (gt.fusion == 'conflict_aware'). Default: sum.
    ...
    def forward(self, batch):
        ...
        if self.self_attn is not None:
            ...
            if self.global_model_type == 'Transformer':
                ...
                # GNN-guided priority serialization: MLP 输入 = h_local + sinusoidal(dist) + is_root
                h_local = h_out_list[0] if h_out_list else h
                has_dist = hasattr(batch, 'dist_from_root') and batch.dist_from_root is not None
                has_root = hasattr(batch, 'is_target_root') and batch.is_target_root is not None
                if has_dist and has_root and batch.dist_from_root.shape[0] == h_local.shape[0]:
                    dist = batch.dist_from_root.to(h_local.dtype)
                    pe_dist = _sinusoidal_dist_encoding(
                        dist, self.gnn_priority_pe_dim, h_local.device
                    )
                    root_feat = batch.is_target_root.unsqueeze(-1).to(h_local.dtype)
                    mlp_in = torch.cat([h_local, pe_dist, root_feat], dim=-1)
                else:
                    # 无 dist/root 时用零填充，保持 MLP 输入维度一致
                    pe_dist = torch.zeros(
                        h_local.shape[0], self.gnn_priority_pe_dim,
                        device=h_local.device, dtype=h_local.dtype
                    )
                    root_feat = torch.zeros(h_local.shape[0], 1, device=h_local.device, dtype=h_local.dtype)
                    mlp_in = torch.cat([h_local, pe_dist, root_feat], dim=-1)
                scores_i = torch.sigmoid(self.gnn_priority_mlp(mlp_in).squeeze(-1))
                batch.gnn_priority_scores = scores_i
                # 有 dist_from_root 时用 LexSort（GPU 向量化，dist 主导），否则回退到 heapq BFS
                _use_lexsort = (
                    hasattr(batch, 'dist_from_root') and batch.dist_from_root is not None
                    and batch.dist_from_root.shape[0] == scores_i.shape[0]
                )
                if _use_lexsort:
                    dist = batch.dist_from_root.to(scores_i.dtype)
                    batch_idx = batch.batch.float().to(device=scores_i.device)
                    h_ind_perm = gnn_priority_lexsort(batch_idx, scores_i, dist)
                else:
