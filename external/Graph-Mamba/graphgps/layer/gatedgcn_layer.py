...
class GatedGCNLayer(pyg_nn.conv.MessagePassing):
    ...
    def forward(self, batch):
        x, e, edge_index = batch.x, getattr(batch, "edge_attr", None), batch.edge_index
        src, dst = edge_index[0], edge_index[1]
        # 边特征须与节点特征同维（编码后为 dim_inner）。无 edge_attr 或维度不一致时用两端点节点特征均值
        if e is None or e.size(-1) != x.size(-1):
            e = (x[src] + x[dst]) / 2.0
            batch.edge_attr = e
