...
try:
    ...
    FlopsProfiler = None  # optional: only needed when if_flop=True
#from torch.autograd import profiler
from torch.profiler import profile, record_function, ProfilerActivity


def _device():
    """兼容新版 PyG：无 cfg.device 时使用 cfg.accelerator。"""
    return torch.device(getattr(cfg, "device", getattr(cfg, "accelerator", "cuda")))
...
def _add_gnn_priority_aux_loss(loss, batch):
    """Add auxiliary BCE loss for GNN-guided BFS: train MLP to predict tree vs noise.

    The priority BFS is non-differentiable, so the MLP receives no gradient from the
    main task. This auxiliary loss uses GT node labels (1=tree, 0=noise) to supervise
    the scores, enabling the MLP to learn correct tree likelihood for ordering.
    """
    if not (hasattr(batch, 'gnn_priority_scores') and batch.gnn_priority_scores is not None):
        return loss
    if not (hasattr(batch, 'y') and batch.y is not None):
        return loss
    if 'Mamba_GNNPriorityBFS' not in getattr(cfg.gt, 'layer_type', ''):
        return loss
    aux_weight = getattr(cfg.gt, 'gnn_priority_aux_weight', 0.5)
    scores = batch.gnn_priority_scores
    labels = batch.y.squeeze(-1).float()
    if labels.dim() > 1:
...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    if_select = False
    if if_flop:
        prof = FlopsProfiler(model, None) if FlopsProfiler is not None else None
        #profile_step = 0
        total_flop_s = 0.
        sample_count = 0
        if if_select:
    ...
    for iter, batch in enumerate(loader):
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            split_idx = loader.dataset.split_idx['train'].to(_device())
        ...
        # 若 loss 为 NaN/Inf 则跳过 backward/step，避免梯度污染导致后续全为 NaN
...
def eval_epoch(logger, loader, model, split='val'):
    ...
    for batch in loader:
        ...
        if cfg.dataset.name == 'ogbg-code2':
            ...
            index_split = loader.dataset.split_idx[split].to(_device())
