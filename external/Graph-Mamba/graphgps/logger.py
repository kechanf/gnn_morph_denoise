...
class CustomLogger(Logger):
    ...
    def classification_binary(self):
        ...
        if true.shape[0] < 1e7:  # AUROC computation for very large datasets is too slow.
            # TorchMetrics AUROC on GPU if available（兼容新旧版本：新版本用 task=，旧版本用 pos_label=）
            _p = pred_score.to(_device()).squeeze(-1) if pred_score.dim() > 1 else pred_score.to(_device())
            _t = true.to(_device())
            try:
                auroc_score = auroc(_p, _t, task="binary")
            except TypeError:
                auroc_score = auroc(_p, _t)
            if self.test_scores:
                # SK-learn version.
                try:
    ...
    def classification_multi(self):
        ...
        if true.shape[0] < 1e7:
            ...
            res['auc'] = reformat(auroc(pred_score.to(_device()),
                                        true.to(_device()).squeeze(),
    ...
    def classification_multilabel(self):
        ...
        true = true.to(_device())
        pred_score = pred_score.to(_device())
