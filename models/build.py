...
支持：gcn, gat, sage, gin；以及子模块中的可选模型（如 gcn_residual）。
"""
import torch
...
from models.custom_models import get_custom_model
...
def build_model(
    ...
    model_type: "gcn" | "gat" | "sage" | "gin" | 或 custom_models 中的类型（如 "gcn_residual"）
    num_layers: 卷积层数 (>= 1)
    """
    ...
    # 子模块中的新模型（不修改上方现有实现，仅在此通过参数调用）
    try:
        return get_custom_model(
            model_type, in_channels, hidden_channels, out_channels, num_layers, dropout
        )
    except ValueError:
        pass
